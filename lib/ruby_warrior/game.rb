  ...
  class Game
    def start
      ...
      if File.exist?(Config.path_prefix + '/.profile')
        ...
        FileUtils.mv(Config.path_prefix + '/ruby-warrior', Config.path_prefix + '/rubywarrior') if File.exist?(Config.path_prefix + '/ruby-warrior')
    ...
    def play_normal_mode
      ...
      elsif current_level.number.zero?
        prepare_next_level
    ...
    def play_current_level
      ...
      if current_level.passed?
        ...
        UI.puts(current_level.clue.hard_wrap) if !Config.skip_input? && current_level.clue && UI.ask('Would you like to read the additional clues for this level?')
      end
      continue
    end
    ...
    def tower_paths
      Dir[File.expand_path('../../towers/*', __dir__)]
    end
    ...
    def final_report
      return unless profile.calculate_average_grade && !Config.practice_level
